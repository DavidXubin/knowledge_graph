...
var graphShapes = (function(){
	...
	function node_color(d){
		const color = set_node_color(d, false);
		if (color) {
			return color;
		}

		if (colored_prop!=="none" && node_code_color.length > 0){
	...
	function show_link_label(d) {
		//if (get_graph_default_display_properties(d.label).length === 0) {
		if (true) {
			return d.label
		} else {
			return ""
	...
	function decorate_link(edges, edgepaths, edgelabels){
		...
		edgelabels_deco.append('textPath')
			...
			.attr("startOffset", "50%")
			.text(function (d) {return show_link_label(d)});
	...
	function add_checkbox_prop(item, selected_items){
		...
		if (item === 'edges'){
			item_properties = graphioGremlin.get_edge_properties();
		} else if (item === 'nodes'){
			item_properties = graphioGremlin.get_node_properties();
		}
		item_properties = item_properties.concat(g_flatten_graph_display_properties);
		item_properties = Array.from(new Set(item_properties));

		for (const prop_idx in item_properties){
			const prop_name = item_properties[prop_idx];
			const prop_id_nb = prop_idx;

			attach_property(selected_items, prop_name, prop_id_nb, item, true);
		}		
	}
	...
	function colorize(prop_name){
		...
		if (prop_name === "none"){
			...
			d3.selectAll(".base_circle").style("fill", set_node_color);
			d3.selectAll(".Pin").style("fill", set_pin_node_color);
		}
		else{
			...
			d3.selectAll(".base_circle").style("fill",function(d){
				if (typeof d.properties[prop_name] !=="undefined"){
					return set_node_color(d);
				}
				return node_color(d);
			});
			d3.selectAll(".Pin").style("fill",function(d){
				if (typeof d.properties[prop_name] !=="undefined"){
					return set_pin_node_color(d);
				}
				return node_color(d);
			});
		}
	...
	function set_node_color(node, set_all=true) {
		if (traversal_source === "risk_radar") {
			if (node.properties["tag"][0].value === "金融单(风险单)") {
				return "#ff0000";
			} else if (node.properties["tag"][0].value === "金融单(正常单)") {
				return default_node_color;
			}
		} else if (traversal_source === "loan_agent") {
			const tag = node.properties["tag"][0].value;

			if (node.label === "借款用户") {

				if (tag.indexOf("失联") >= 0) {
					return "#000000";
				} else if (tag.indexOf("坏客户") >= 0) {
					return "#800080";
				} else if (tag.indexOf("首逾30") >= 0) {
					return "#ff0000";
				} else if (tag.indexOf("离职") >= 0) {
					return "#ffa500";
				} else if (tag.indexOf("放款") >= 0 && tag.indexOf("未放款") < 0) {
					return "#0000ff";
				}
			} else if (node.label === "团队" || node.label === "门店") {
				if (tag.indexOf("高风险") >= 0) {
					return "#ff0000";
				}
			}
		...
		if (set_all) {
			return color_palette(node_code_color(node.label));
		} else {
			return null;
		}
	}

	function set_pin_node_color(node){
		if (traversal_source === "risk_radar") {
			if (node.properties["tag"][0].value === "金融单(风险单)") {
				return "#ff00ff"
			} else if (node.properties["tag"][0].value === "金融单(正常单)") {
	...
	return {
		...
		colorize : colorize,
		add_checkbox_prop: add_checkbox_prop
	};
