...
public class EdgeLoaderWorker extends Worker {
    ...
    private void acceptRecord(Map<String, String> record) throws Exception {
        ...
        if (!node_1.hasNext()) {
            log.warn("Left " + record.get(leftEdgeFieldName) + " has empty node");
        } else {
            log.warn("Left " + record.get(leftEdgeFieldName) + " has next node");
        }

        if (!node_2.hasNext()) {
            log.warn("Right " + record.get(rightEdgeFieldName) + " has empty node");
        } else {
            log.warn("Right " + record.get(rightEdgeFieldName) + " has next node");
        }
