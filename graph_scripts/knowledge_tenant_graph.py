...
company_schema = make_node_schema("开发商", gra_company,
                                  comp_index_properties = ["项目公司统一社会信用代码"],
                                  mix_index_properties = ["tag", "项目公司名称"])

company_mapper = make_node_mapper("开发商", gra_company)

dump_schema(company_schema, company_mapper, "company")

gra_company.to_csv(graph_type + "company/gra_开发商.csv", sep=',', header=True, index=False)
##################################################################################################################
