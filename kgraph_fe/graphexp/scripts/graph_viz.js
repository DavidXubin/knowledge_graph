...
var graph_viz = (function () {
	...
	function refresh_data(d, center_f, with_active_node) {
		...
		var focus_node_data = d3.selectAll(".focus_node").data();
		if (focus_node_data.length > 0){
			for (var i in focus_node_data) {
				if (focus_node_data[i].id == with_active_node) {
					infobox.display_info(focus_node_data[i]);
				}
			}
		}
		//////////////////////////////////
